(function(){var SECTOR_INSET_PCT=0.4;var LOGO_MIN_W=80;var LOGO_MIN_H=50;var TEXT_MIN_W=60;var TEXT_MIN_H=46;function fmtPct(value){var abs=Math.abs(value);var rounded=Math.round(abs*10)/10;if(rounded>=100){return String(Math.round(value));}
...
row.logoHFactor.toFixed(3)+";";inner.appendChild(img);}
...
function boot(){var figures=document.querySelectorAll("figure.treemap");if(!figures.length)return;var relayout=debounce(function(figure){layoutFigure(figure);},80);for(var fi=0;fi<figures.length;fi++){var figure=figures[fi];layoutFigure(figure);var canvas=figure.querySelector(".treemap__canvas");if(!canvas)continue;if(typeof ResizeObserver!=="function")continue;var observer=new ResizeObserver(function(){relayout(figure);});observer.observe(canvas);}}
