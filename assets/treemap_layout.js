...
return anchors;}
function foldRowsByAnchors(rows,anchors,otherSector){if(!anchors.length)return rows;var toFold=[];for(var i=0;i<rows.length;i++){var row=rows[i];if(!row.foldedLabels&&anchors.indexOf(row.anchor)>=0){toFold.push(row);}}
if(!toFold.length)return rows;var remainingReal=[];for(var ri=0;ri<rows.length;ri++){var r=rows[ri];if(!r.foldedLabels&&toFold.indexOf(r)<0)remainingReal.push(r);}
...
var batchWeight=0;var batchLabels=[];for(var bi=0;bi<toFold.length;bi++){batchWeight+=toFold[bi].weight;batchLabels.push(toFold[bi].shortTicker||stripExchange(toFold[bi].ticker));}
var existingOther=null;for(var oi=0;oi<next.length;oi++){if(next[oi].foldedLabels){existingOther=next[oi];break;}}
if(!existingOther){next.push({ticker:"",name:"Other",sector:otherSector,weight:batchWeight,logoUrl:"",logoWFactor:1,logoHFactor:1,anchor:"",shortTicker:"",foldedLabels:batchLabels,});}else{existingOther.weight+=batchWeight;existingOther.foldedLabels=existingOther.foldedLabels.concat(batchLabels);}
return next;}
function mergeSmallIntoOther(rows,canvasW,canvasH,otherSector){var rowsList=rows.slice();var maxPasses=rowsList.length+1;for(var pass=0;pass<maxPasses;pass++){var layout=layoutRows(rowsList);var toFold=[];for(var li=0;li<layout.length;li++){var entry=layout[li];if(!entry.row.foldedLabels&&tileShouldFold(entry.tile,canvasW,canvasH)){toFold.push(entry.row);}}
if(!toFold.length)return rowsList;var remainingReal=[];for(var ri=0;ri<rowsList.length;ri++){var row=rowsList[ri];if(!row.foldedLabels&&toFold.indexOf(row)<0){remainingReal.push(row);}}
...
var batchWeight=0;var batchLabels=[];for(var bi=0;bi<toFold.length;bi++){batchWeight+=toFold[bi].weight;batchLabels.push(toFold[bi].shortTicker||stripExchange(toFold[bi].ticker));}
var existingOther=null;for(var oi=0;oi<next.length;oi++){if(next[oi].foldedLabels){existingOther=next[oi];break;}}
if(!existingOther){next.push({ticker:"",name:"Other",sector:otherSector,weight:batchWeight,logoUrl:"",logoWFactor:1,logoHFactor:1,anchor:"",shortTicker:"",foldedLabels:batchLabels,});}else{var mergedLabels=existingOther.foldedLabels.concat(batchLabels);existingOther.weight+=batchWeight;existingOther.foldedLabels=mergedLabels;}
rowsList=next;}
return rowsList;}
...
sectorVar+");");}
function buildAggregatedTile(row,tile,meta,sectorVar){var labelPct=fmtPct(row.weight);var count=row.foldedLabels.length;var tickersBlurb=[];for(var i=0;i<row.foldedLabels.length;i++){tickersBlurb.push(row.foldedLabels[i]);}
